...
const AddItem = () => {
  ...
  // Fetch the logged-in user's userId from localStorage
  const loggedInUserId = localStorage.getItem('userId') || null; // Default to null if not found
  ...
  const handleSubmit = (e) => {
    ...
    if (!item || !quantity || !unit || !category) {
      ...
      if (!loggedInUserId) {
        setError("No user logged in, cannot add item.");
        return;
      }
      // Add the logged-in user's userId to the state before dispatching
      const newItem = {
        ...state,
        userId: loggedInUserId // Attach the userId to the new item
      };

      dispatch(addItem(newItem)); // Dispatch with the updated state
      navigate('/home');
      setError("");
    }
  };
      ...
      <div className="row">
        ...
        <div className="col-8">
          <div className="card">
            <div className="card-body">
              <div className="card">
                <h5 className="card-header d-flex justify-content-start">Add New Item</h5>
                  ...
                  <form className="row g-3" onSubmit={handleSubmit}>
                    ...
                    <div className="card-footer bg-body-tertiary px-3">
                      <div className='btn-group'>
                        <button onChange={handleInputChange} type="submit" className="btn btn-outline-success" data-mdb-ripple-init>ADD</button>
                        <Link to={'/home'} type="button" className="btn btn-outline-dark" data-mdb-ripple-init>BACK</Link>
                      </div>
                    </div>
                  </form>
                </div>
